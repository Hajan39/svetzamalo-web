	...
	return (
		<div className="relative">
			<button
				...
				className={`flex items-center gap-2 rounded-full px-3 py-2 text-sm font-medium transition-colors hover:bg-background-secondary ${className}`}
			...
			{isOpen && (
				<>
					...
					{/* Dropdown */}
					<div className="absolute right-0 top-full z-50 mt-2 min-w-32 rounded-2xl border border-border bg-background shadow-lg">
						{SUPPORTED_LOCALES.map((localeOption) => (
							<button
								key={localeOption.code}
								type="button"
								onClick={() => handleLanguageChange(localeOption.code)}
								className={`w-full flex items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-background-secondary ${
