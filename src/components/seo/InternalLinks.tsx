...
export function CountryToArticleLink({
  ...
  const articlePath = locale === 'en' ? `/articles/${article.slug}` : `/${locale}/articles/${article.slug}`
        ...
        <div className="flex items-center justify-between mb-3">
          {showType && (
            <span className="inline-block text-xs font-medium text-primary bg-primary-light px-2 py-1 rounded">
              {t(`articleTypes.${article.articleType}`, article.articleType.replace('-', ' '))}
            </span>
          )}
        </div>
 ...
 */
export function RelatedDestinationLink({ destination, variant = 'card' }: RelatedContentLinkProps) {
  if (variant === 'inline') {
    return (
      <Link
        to="/destinations/$slug"
...
export function ContentNavigation({ previous, next }: ContentNavigationProps) {
  ...
  return (
    <nav aria-label="Content navigation" className="flex justify-between items-center py-8 border-t border-border mt-12">
      <div className="flex-1">
        {previous && (
          ...
          >
            ...
            <div>
              <div className="text-sm text-foreground-muted uppercase tracking-wide">Previous</div>
              <div className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">
                {previous.title}
              </div>
            </div>
        ...
        {next && (
          ...
          >
            <div className="text-right">
              <div className="text-sm text-foreground-muted uppercase tracking-wide">Next</div>
              <div className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">
                {next.title}
              </div>
            </div>
...
export function shouldLinkToDestination(content: string, destinationName: string): boolean {
  // Don't overlink - only link first mention in intro/content
  const mentions = content.toLowerCase().split(destinationName.toLowerCase()).length - 1
  return mentions <= 1
}

export function createInternalLinkAnchors(content: string): Array<{
  text: string
  href: string
