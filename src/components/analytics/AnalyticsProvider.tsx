...
import { SITE_CONFIG, ANALYTICS_CONFIG, EXTERNAL_SERVICES } from '@/lib/constants'
...
interface AnalyticsContextType {
  ...
  // Utility methods
  canTrack: () => boolean
  isTrackingEnabled: (feature: keyof Pick<AnalyticsConfig, 'trackPageViews' | 'trackScrollDepth' | 'trackCustomEvents'>) => boolean
}
...
export function AnalyticsProvider({
  ...
  const isTrackingEnabled = useCallback((feature: keyof Pick<AnalyticsConfig, 'trackPageViews' | 'trackScrollDepth' | 'trackCustomEvents'>) => {
  ...
  const trackPageView = useCallback((data: PageViewData) => {
    ...
    //   plausible('pageview')
    // }
  }, [isTrackingEnabled, config.provider])
  ...
  const trackScrollDepth = useCallback((data: ScrollDepthData) => {
  ...
  useEffect(() => {
    const storedConsent = localStorage.getItem('analytics-consent') as ConsentStatus | null
...
export function useAnalyticsConsent() {
  const { consentStatus, grantConsent, denyConsent, resetConsent, canTrack } = useAnalytics()
...
export function getAnalyticsConfig(): AnalyticsConfig {
  ...
  return {
    ...DEFAULT_CONFIG,
    ...ANALYTICS_CONFIG,
    isEnabled: enableAnalytics && (isProduction || ANALYTICS_CONFIG.provider === 'google-analytics'),
