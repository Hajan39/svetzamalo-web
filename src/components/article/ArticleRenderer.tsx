 ...
 */
function StructuredArticleRenderer({
	contentBlocks,
}: {
	contentBlocks: ContentBlock[];
}) {
...
function ImageBlockRenderer({
	...
	return (
		<figure className="my-8">
			...
			{block.caption && (
				<figcaption
					className="text-sm text-foreground-muted mt-3 text-center italic"
					itemProp="caption"
				>
					{block.caption}
				</figcaption>
 ...
 */
function TipBlockRenderer({
	block,
}: {
	block: ContentBlock & { type: "tip" };
}) {
	const styles = {
		tip: "bg-accent-light",
...
function ListBlockRenderer({
	...
	return (
		<Component
			className={`my-6 ml-6 ${block.style === "numbered" ? "list-decimal" : "list-disc"} space-y-2`}
		>
			{block.items.map((item) => (
				<li
					key={item}
					className="text-lg text-foreground-secondary leading-relaxed pl-2"
				>
					{item}
				</li>
			))}
