...
export function transformStrapiDestination(
	...
	const cover =
		strapiDest.cover?.data ?? strapiDest.cover ?? strapiDest.heroImage?.data;
	const coverUrl =
		(cover as { url?: string; attributes?: { url?: string } })?.url ??
	...
	return {
		...
		type:
			strapiDest.destinationType ||
			(strapiDest.type as Destination["type"]) ||
			"country",
		...
		heroImage: coverUrl
			? {
					url: coverUrl.startsWith("http")
						? coverUrl
						: `${STRAPI_BASE.replace(/\/$/, "")}${coverUrl.startsWith("/") ? "" : "/"}${coverUrl}`,
					alt:
						(
							cover as {
								alternativeText?: string;
								attributes?: { alternativeText?: string };
							}
						)?.alternativeText ??
						(cover as { attributes?: { alternativeText?: string } })?.attributes
							?.alternativeText ??
						"",
		...
		introHtml: strapiDest.intro?.length
			? strapiBlocksToHtml(
					strapiDest.intro as Parameters<typeof strapiBlocksToHtml>[0],
				)
			: undefined,
...
function getStrapiCoverImage(
	strapiArticle: StrapiArticle,
): Article["coverImage"] {
	const toFullUrl = (url: string) =>
		url.startsWith("http")
			? url
			: `${STRAPI_BASE.replace(/\/$/, "")}${url.startsWith("/") ? "" : "/"}${url}`;

	// Strapi v5: cover is returned directly; Strapi v4: cover is in .data
	const cover = (strapiArticle.cover?.data ?? strapiArticle.cover) as
		| {
				url?: string;
				attributes?: { url?: string; alternativeText?: string };
				alternativeText?: string;
	...
	if (coverUrl) {
		return {
			...
			alt:
				...
				cover?.attributes?.alternativeText ??
				strapiArticle.title ??
				"",
		};
	}
...
export function transformStrapiArticle(strapiArticle: StrapiArticle): Article {
	...
	const destinationId =
		...
		countryRel?.data?.id?.toString() ||
		countryRel?.documentId ||
		countryRel?.id?.toString() ||
		"";
	...
	return {
		...
		destinationId,
		articleType:
			(strapiArticle.articleType as Article["articleType"]) ??
			"destination-guide",
